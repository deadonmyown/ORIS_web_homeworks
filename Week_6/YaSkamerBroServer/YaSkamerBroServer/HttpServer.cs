...
namespace YaSkamerBroServer
{
    public class HttpServer : IDisposable
    {
        ...
        private async Task<bool> MethodHandler(HttpListenerContext _httpContext)
        {
            ...
            var res = await RouteTree.TryNavigate(new HttpMethod(request.HttpMethod), request.RawUrl, bodyRet, null, bodyParams);
    ...
    private void RegisterControllers()
        {
            ...
            var controllers = assembly.GetTypes()
                ...
                .Select(tuple => (tuple.type, string.IsNullOrEmpty(tuple.Item2.ControllerName) 
                    ? tuple.type.Name.ToLower().Replace("controller", "") : tuple.Item2.ControllerName));
